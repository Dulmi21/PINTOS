...
int pintos_init(void)
{
  ...
  else
  {
    ...
    {
      if (input_text())
        break; // returns 1 if needed to shutdown, else 0
    }
  }
...
int input_text()
{
  input_init(); // initialize the buffer
  ...
  // actions on user inputs
  // whoami
  if (strcmp(textInput, "whoami") == 0)
  {
    printf("\nName- Y.M.D.P. YAPA , Index number- 210735U\n");
    return 0;
  }
  // shutdown
  if (strcmp(textInput, "shutdown") == 0)
  {
    shutdown_power_off();
    printf("\n");
    return 1;
  ...
  // time
  if (strcmp(textInput, "time") == 0)
  {
    printf("\nTime since started (in seconds): %" PRIu64 "\n", timer_ticks, "\n");
    return 0;
  }
  // ram
  if (strcmp(textInput, "ram") == 0)
  {
    printf("Pintos booting with %'" PRIu32 " kB RAM...\n", init_ram_pages * PGSIZE / 1024);
    return 0;
  }
  // thread
  if (strcmp(textInput, "thread") == 0)
  {
    printf("\n");
    thread_print_stats();
    return 0;
  ...
  // priority
  if (strcmp(textInput, "priority") == 0)
  {
    printf("\n%d", thread_get_priority());
    printf("\n");
    return 0;
  }
  // exit
